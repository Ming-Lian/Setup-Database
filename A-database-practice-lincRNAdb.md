...
if($submit){
	...
	if(empty($log2FoldChange)&&empty($p_value)&&empty($q_value)){
		...
		if($result->num_rows > 0){
			...
			while($row = $result->fetch_array()){
        			echo "<tr><td>".$row['RefSeq']."</td><td>".$row['baseMean']."</td><td>".$row['log2FoldChange']."</td><td>".$row['lfcSE']."</td><td>".$row['stat']."</td><td>".$row['pvalue']."</td><td>".$row['padj']."</td><td><a href=\"Updata_DiffExp.php?id=".$row['id']."tbname=DIffexp_result\" style=\"padding:2px;\">Update</a><a href=\"Barplox_profile.php\" style=\"padding:2px;\">Barplox</a></td></tr>\n";
    		}
		}else{
    			echo "未检索到满足条件的记录!";
		}
